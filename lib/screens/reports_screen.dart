    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                ...
                try {
                  await PdfReportService.generateAndOpenWeeklyReport(
                    auth,
                    data,
                  );
                  if (context.mounted) {
          ...
          // Heatmap
          const Text(
            'Heatmap — RED events by weekday × hour',
            style: TextStyle(
              color: Color(0xFFe8edf4),
              fontSize: 14,
              fontWeight: FontWeight.w600,
  ...
  Widget _buildTrendChart(List<DateTimeChartItem> items) {
    if (items.isEmpty) {
      return const SizedBox(
        height: 200,
        child: Center(
          child: Text('No data', style: TextStyle(color: Color(0xFF8b9cb3))),
        ),
      );
    }
                      ...
                      (h) => Row(
                        children: List.generate(5, (d) {
                          ...
                          final intensity = maxCount > 0
                              ? cell.count / maxCount
                              : 0.0;
                          final r = (239 * intensity + 34 * (1 - intensity))
                              .toInt();
                          final g = (68 * intensity + 197 * (1 - intensity))
                              .toInt();
                          return Container(
                            width: 24,
                            height: 24,
